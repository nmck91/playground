...
import { CHILD_REWARDS, PARENT_REWARDS, FamilyMember } from '../models/types';
                  ...
                  <div class="mb-3">
                    ...
                    <div class="w-full bg-slate-200 rounded-full h-2 sm:h-3 overflow-hidden">
                      <div class="bg-gradient-to-r from-brand-accent to-brand-secondary h-full transition-all"
                           [style.width.%]="(selectedMemberStars / reward.requiredStars) * 100">
                      </div>
                    </div>
                  </div>
                </div>
...
export class RewardsViewComponent implements OnInit {
  ...
  toggleViewMode(): void {
    this.viewMode = this.viewMode === 'kids' ? 'parents' : 'kids';
  ...
  getRewardsForSelectedMember(): any[] {
  ...
  claimReward(rewardId: string): void {
    ...
    alert('🎉 Reward claimed! (Coming soon - add to claimed rewards list)');
  }
}
